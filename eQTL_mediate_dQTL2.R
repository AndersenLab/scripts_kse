...
eQTL_mediate_dQTL <- function(peak, probe, phenodf, scaled = T, lm = TRUE) {
    ...
    if(lm) {
        ### LINEAR MODELS ###
        # genotype/phenotype with regression for expression
        regressed <- pheno %>% 
            do(fit = lm(phenotype ~ expression, .))%>% 
            broom::augment(fit) %>% 
            dplyr::ungroup() %>% 
            dplyr::left_join(pheno, .) %>% 
            dplyr::select(strain, phenotype, expression, set, geno, pheno_resid = .resid, pheno_fit = .fitted)
        
        
        regressed2 <- pheno %>%
            do(fit2 = lm(geno ~ expression, .)) %>%
            broom::augment(fit2) %>% 
            dplyr::ungroup() %>% 
            dplyr::left_join(regressed, by = c("expression", "geno")) %>% 
            dplyr::select(strain, phenotype, expression, set, geno, pheno_resid, pheno_fit, geno_resid = .resid, geno_fit = .fitted)
        ...
        model.y <- lm(pheno_resid ~ geno, data = regressed2)
