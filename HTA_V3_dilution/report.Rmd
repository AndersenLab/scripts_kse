...
cat('\n')
if(dose == T) {
    drug_inputs <- data.frame(drug_name = NA, drug_stock = NA, control_name = NA, drug_dose = NA, drug_plates = NA, wells = NA)
    for(d in 1:drugs) {
    ...
    drug_inputs <- data.frame(drug_name = NA, drug_stock = NA, control_name = NA, drug_dose = NA, drug_plates = NA, control_plates = NA, wells = NA)
    for(d in 1:drugs) {
        drug_inputs <- rbind(drug_inputs, c(get(glue::glue("drug_name{d}")),
                             get(glue::glue("drug_stock{d}")),
                             get(glue::glue("control_name{d}")),
...
knitr::kable(dosetable[[3]], format = "markdown")
...
if(dose == T) {
    ...
    knitr::kable(dosetable[[1]], format = "markdown")
}
