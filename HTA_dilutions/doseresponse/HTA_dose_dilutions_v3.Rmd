...
for (i in 1:nrow(curves)){
    ...
    for (j in 1:length(tenfolds)) {
        ...
        count[j] <- sum(as.numeric(as.character(recipe$ul)) >=.5 & as.numeric(as.character(recipe$ul)) <= (1200*reps)*3/100)
    }
    ...
    for (k in 1:nrow(recipe)){
        if (as.numeric(as.character(recipe$ul[k])) <= (1200*reps)*3/100 | as.numeric(as.character(recipe$ul[k])) == 0){
            goodrows <- append(goodrows, as.numeric(k))
        }
        if (as.numeric(as.character(recipe$ul[k])) > (1200*reps)*3/100){
    ...
    if (length(higherConcs) > 0){
        ...
        for (l in 1:length(higherStocks)){
            ...
            if (all(highRecipe$ul >= .5) & all(highRecipe$ul <= (1200*reps)*3/100)){
    ...
    recipe$lysate <- (1200*reps) - (1200*reps)*3/100
    ...
    if(exists("highRecipe")){
        # lysate is always going to be 1200 - 12 to keep the solvent at 1%
        highRecipe$lysate <- (1200*numplate) - (1200*numplate)*3 / 100
        highRecipe$ul <- as.numeric(as.character(highRecipe$ul))
